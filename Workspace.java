...
class Workspace extends JPanel implements MouseListener, MouseMotionListener, Observer {
	...
	private static String connectionType = "tsp";
	private ArrayList<Connection> connectionsOfSelectedCity = new ArrayList<Connection>();
	private ArrayList<Integer> connectionsTypeOfSelectedCity = new ArrayList<Integer>();
	 ...
	 * 
	 * @param graphics object of class Graphics used to draws the cities in GUI 
	 * and connect them with line.
	 */	
		...
		try {			
			...
			if (clickedCity != null) {
				...
				if (action.equals("move")) {
					...
					bb.addConnection(fc.createConnection(connectionSize, clickedCity.getIndex(), 
							-1, (int) clickedCity.center().getX(), 
							(int) clickedCity.center().getY(),
							(int) clickedCity.center().getX(), 
							(int) clickedCity.center().getY()));
					messageDisplayer.setMessageColor(Color.CYAN);
	...
	public void mouseDragged(MouseEvent e) {
		if (action.equals("move")) {
			...
			if (clickedCity != null) {
				...
				messageDisplayer.setMessage("Hey, that's great! "
						+ "You are trying to make a new connection.");
				repaint();
			}
		}
		dragged = true;
				...
				if (action.equals("move")) {
					...
					if (!dragged) {
						if (pressOut) {
							...
							JOptionPane.showMessageDialog(null, 
									"City already exists where you want to create a new city.");
						}
					}
				} else if (action.equals("connect")) {
					...
					if (tempCity != null && clickedCity != null) {
						if (!bb.containsConnection(clickedCity.getIndex(), tempCity.getIndex())) {
							bb.getConnections().get(connectionSize - 1).setCity2Index(
									tempCity.getIndex());
	...
	/**
	 * This function is used to specify the operations to be performed when 
	 * mouse is moved.
	 * 
	 * @param e object of class MouseEvent containing details of mouse events.
	 */
	...
	/**
	 * This function specifies the operations to be performed when TSPSovler 
	 * is done executing.
	 * 
	...
	public void update(Observable o, Object arg) {
		...
		if (connectionType.equals("tsp")) {
			int[] shortestPath = bb.getShortestPath();
			if (shortestPath == null) {
				messageDisplayer.setMessageColor(Color.RED);
				messageDisplayer.setMessage("Please restart the application. Something went wrong.");
			} else {
				...
				} else if (shortestPath.length == 1 && shortestPath[0] == -1) {
					messageDisplayer.setMessageColor(Color.GRAY);
					messageDisplayer.setMessage("Please create few cities (i.e more than 1) to see TSP solver in action.");
				}
			}
		} else if (connectionType.equals("cluster")) {
			ArrayList<Integer>[] clusters = bb.getClusters();
			int count = 0;

			if (clusters.length == 1 && clusters[0].get(0) == -1) {
				messageDisplayer.setMessageColor(Color.GRAY);
				messageDisplayer.setMessage("Please create few cities (i.e at least 1) to see K-Means clustering algorithm in action.");
			} else {
	 ...
	 * 
	 * @return connectionType, name of algorithm to be executed.
	 */
	...
	private void mouseDraggedMove(MouseEvent e) {
		...
		try {
			if (!pressOut) {
				...
				for (int itr = 0; itr < connectionsOfSelectedCity.size() ; itr++) {
					if (connectionsTypeOfSelectedCity.get(itr) == 1) {
						connectionsOfSelectedCity.get(itr).setCor1(
								(int) clickedCity.center().getX(), 
								(int) clickedCity.center().getY());
					} else if (connectionsTypeOfSelectedCity.get(itr) == 2) {
						connectionsOfSelectedCity.get(itr).setCor2(
								(int) clickedCity.center().getX(), 
								(int) clickedCity.center().getY());
					}
				}
	...
	private void createACity(MouseEvent e) {
		MessageDisplayer messageDisplayer = MessageDisplayer.getInstance();
		try {
			String cityName = JOptionPane.showInputDialog(this, 
					"Please enter the name of city you want to add : ", null);
			...
			if (cityName != null && cityName.trim().length() > 0) {
				if (cities.size() > 0) {
					...
					for (int itr = 0; itr < cities.size(); itr++) {
						if (cities.get(itr).getCityName().equals(cityName)) {
							uniqueName = false;
							break;
						}
					}
					if (uniqueName) {
						bb.addCity(fc.createCity(cityName,cities.size(), 
								e.getX(), e.getY()));
					}
					else {
						JOptionPane.showMessageDialog(null, 
								"City's name should be unique.");
						createACity(e);
					}
				} else {
            	...
            	messageDisplayer.setMessageColor(Color.RED);
    			messageDisplayer.setMessage("OOPs! You just missed creating a NEW city.");
            }
	...
	private void findConnections(int cityIndex) {
		Blackboard bb = Blackboard.createInstance();
		ArrayList<Connection> allConnection = bb.getConnections();
		connectionsOfSelectedCity.clear();
		connectionsTypeOfSelectedCity.clear();

		for (int itr = 0; itr < allConnection.size(); itr++) {
			if (allConnection.get(itr).getCity1Index() == cityIndex) {
				connectionsTypeOfSelectedCity.add(1);
				connectionsOfSelectedCity.add(allConnection.get(itr)); 
			} else if (allConnection.get(itr).getCity2Index() == cityIndex) {
				connectionsTypeOfSelectedCity.add(2);
				connectionsOfSelectedCity.add(allConnection.get(itr)); 
			}
		}
	}
	...
	public void editCity() {
		String[] shapeDecorators = new String[] {"Square", "Circle Square", "Circle Square group", 
				"Square Group"};
		...
		switch(selectedDecorator) {
			...
			case 1:
					newCity = new City(clickedCity.getCityName(), clickedCity.getIndex(),
							 clickedCity.getX(), clickedCity.getY(), selectedSize, 
							 selectedDecorator, selectedColor);
					break;
			case 2:
					newCity = new City(clickedCity.getCityName(), clickedCity.getIndex(),
							 clickedCity.getX(), clickedCity.getY(), selectedSize, 
