...
class TSPNN extends Observable implements Runnable, Handler {
	...
	/**
	 * This function is used to create the object class TSPNN and if it is already created,
	...
	/**
	 * This method calls the function that triggers TSP solving algorithm.
	 */
	...
	private int[] nodeCoordSectionSorter(ArrayList<City> cities, int count, int tspPath[],
			String tspPathStr) {
		try {
			if (count == 0) {
				...
				for (int itrI = 0; itrI < cities.size(); itrI++) {
					...
					if (min > dist && cities.get(latestNodeIndex).getIndex() != 
							cities.get(itrI).getIndex() && 
							!tspPathStr.contains("&" + cities.get(itrI).getIndex() + "&")) {
						min = dist;
